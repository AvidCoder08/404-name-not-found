...
processing_mode = st.sidebar.selectbox(
    "Processing Mode", 
    ["Local (In-Browser)", "Backend API (FastAPI)"],
    index=0
)

backend_url = "http://localhost:8000"
if processing_mode == "Backend API (FastAPI)":
...
with tab1:
    ...
    if analyze_btn:
        ...
        if processing_mode == "Backend API (FastAPI)":
            ...
            with st.spinner(f"Sending data to Backend API ({backend_url})..."):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        st.session_state.suspicion_scores = results.get("suspicion_scores", {})
                        st.session_state.detected_rings = results.get("fraud_rings", [])
                        
                        summary = results.get("summary", {})
                        st.success(f"✅ Backend Analysis Complete! Flagged {summary.get('accounts_flagged')} accounts and {summary.get('fraud_rings_detected')} rings.")
            ...
            # Step 4: Filter high-risk nodes (for subgraph extraction)
            # Note: The backend does everything in one go. Here we do it stepwise.
